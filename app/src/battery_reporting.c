...
int peripheral_batt_report_lvl_listener(const zmk_event_t *eh) {
    ...
    LOG_DBG("Peripheral %d battery HID report: level=%d battery_index=%d",
            ev->source, ev->state_of_charge, ev->source + BAS_PERIPHERAL_INDEX_OFFSET);
...
int central_batt_state_changed_listener(const zmk_event_t *eh) {
    ...
    LOG_DBG("Central battery HID report: level=%d battery_index=%d",
            ev->state_of_charge, BAS_CENTRAL_INDEX);
